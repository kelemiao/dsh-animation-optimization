/**
 * dsh-animation-optimization — client half (v29).
 *
 * v29: the memory-evolve session-review header entry is aligned with the DSH
 * top bar, the review panel is guarded against theme hiding, and its long
 * live flow keeps following the newest content (with user scroll-up pause).
 * Performance passes keep UX identical: tool bodies are skipped by blank-line
 * preprocessing, the document-order list is built once per sync and only when
 * a collapse is pending, and the red-dot sweep runs only on childList changes.
 * v28: renamed from dsh-theme-photo-editorial to dsh-animation-optimization;
 * plugin id, settings slots, style attributes and localStorage keys use the
 * new package name.
